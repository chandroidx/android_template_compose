...
object LocalResultStore {
  ...
  infix fun provides(
    store: ResultStore,
  ): ProvidedValue<ResultStore?> = LocalResultStore.provides(store)
}
 ...
 */
@Composable
fun rememberResultStore(): ResultStore = remember {
  ResultStore()
}
...
class ResultStore {
   ...
   */
  val resultStateMap: SnapshotStateMap<String, MutableState<Any?>> = mutableStateMapOf()

  /**
   * Retrieves the current result of the given resultKey.
   */

  inline fun <reified T> getResultState(resultKey: String = T::class.toString()): MutableState<T?> {
    @Suppress("UNCHECKED_CAST")
    val result = resultStateMap.getOrPut(resultKey) {
      mutableStateOf(null)
    } as MutableState<T?>
